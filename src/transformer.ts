...
function createLiteralExpression(type: ts.Type, typeChecker: ts.TypeChecker) {
	...
	if (type.isNumberLiteral()) return factory.createNumericLiteral(type.value.toString());
}
...
function convertArrayType(type: ts.GenericType, typeChecker: ts.TypeChecker): ts.Expression {
	...
	if (args === undefined || args.length === 0) throw new Error("Array must have type arguments");
...
function convertObjectType(props: ts.Symbol[], typeChecker: ts.TypeChecker): ts.Expression {
	if (props.length === 0) return createMethodCall("interface", [factory.createObjectLiteralExpression([])]);
	...
	if (optionalProps.length !== 0) result.push(handlePropList(optionalProps, true));

	if (nonOptionalProps.length !== 0) result.push(handlePropList(nonOptionalProps));

	return result.length === 1 ? result[0] : createMethodCall("intersection", result);
}
...
function convertEnumType(type: ts.Type, typeChecker: ts.TypeChecker): ts.Expression {
	...
	for (const member of enumDeclaration.members) {
		...
		if (typeof value === "string") result.push(factory.createStringLiteral(value));
		else if (typeof value === "number") result.push(factory.createNumericLiteral(value));
...
export function buildType(type: ts.Type, typeChecker: ts.TypeChecker): ts.Expression {
	...
	if (stringType === "never") throw new Error("Never type transformation is not supported");

	if (type.isClass()) throw new Error("Transformation of classes is not supported");

	// Basic types transformation
	if (["null", "undefined", "void", "unknown"].includes(stringType)) return createPropertyAccess("none");

	if (ROBLOX_TYPES.includes(stringType)) return createPropertyAccess(stringType);
	...
	if (utility.isEnum(type))
		return createMethodCall("enum", [
			factory.createPropertyAccessExpression(factory.createIdentifier("Enum"), stringType),
		]);
	...
	if (type.isStringLiteral()) return createLiteral(factory.createStringLiteral(type.value));

	if (type.isNumberLiteral()) return createLiteral(factory.createNumericLiteral(type.value.toString()));

	if (["string", "number", "boolean", "any", "thread"].includes(stringType)) return createPropertyAccess(stringType);
	...
	try {
		...
		else if (utility.isTupleType(type, typeChecker)) return convertTupleType(type, typeChecker);
		else if (utility.isArrayType(type, typeChecker)) return convertArrayType(type, typeChecker);
		else if (utility.isObjectType(type) || type.isIntersection())
			return convertObjectType(type.getProperties(), typeChecker);
		else if (type.isClassOrInterface()) return convertInterfaceType(type, typeChecker);
	} catch (err) {
